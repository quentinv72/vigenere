...
class VigenereDecipher:
    ...
    def __init__(self,cipher:str,key:Optional[str]=None,key_length:Optional[int]=None):
        if key:
        ...
        self.cipher_representation= [self.alphabet_position[letter] for letter in self.cipher]
    ...
    def guess_key(self, keylength)->list:
        frequencies = dict()
        for i in range(keylength):
            frequencies[i]=self._frequencies_letter(i,keylength)
        possible_keys =  list(itertools.product(*list(frequencies.values())))
        return ''.join(possible_keys[0])

    def _frequencies_letter(self,position,keylength):
        frequency= dict()
